...
def main() -> int:
    ...
    init = commands.add_parser("init"); init.add_argument("vault", type=Path); init.add_argument("--force", action="store_true"); init.add_argument("--agent-name")
    ...
    if args.command == "init":
        print(json.dumps({"created": [str(p) for p in init_vault(args.vault, args.force, args.agent_name)]}, indent=2))
